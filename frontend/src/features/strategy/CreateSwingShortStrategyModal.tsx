...
import { TrendingDown, TrendingUp, RotateCw, Zap, AlertTriangle, DollarSign, TrendingDown as LineChartDown } from 'lucide-react'
...
interface CreateSwingShortStrategyModalProps {
  open: boolean
  onCancel: () => void
  onSuccess: () => void
  editMode?: boolean
...
export default function CreateSwingShortStrategyModal({ open, onCancel, onSuccess, editMode = false, initialData }: CreateSwingShortStrategyModalProps) {
  ...
  const estimatedFee = effectiveAmount ? effectiveAmount * feeRate * 2 : null
  ...
  const fetchCurrentPrice = async (tradingSymbol: string, isUpdate: boolean = false) => {
    try {
      if (isUpdate) setPriceUpdating(true)
      ...
      if (isUpdate) setTimeout(() => setPriceUpdating(false), 300)
    }
  }
    ...
    if (open) {
      ...
      if (editMode && initialData) {
        ...
        form.setFieldsValue({
          ...
          limit_order_offset_pct: params.limit_order_offset_pct || 0.5
        })
      } else {
        form.setFieldsValue({
          initial_amount: 1000,
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => fetchCurrentPrice(symbol, true), 5000)
  ...
  const generateStrategyName = () => {
    ...
    return `${baseCcy}空单 | ${leverage || 5}x | ${dateStr}`
  }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const strategyData = {
        ...
        description: `波段做空 - ${values.leverage}x杠杆 止盈${values.take_profit_pct}% 止损${values.stop_loss_pct}% ${values.use_limit_orders ? '(限价单)' : '(市价单)'}`
      }
    ...
    <Modal
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <TrendingDown size={14} style={{ color: '#ff4d4f' }} />
          <span>{editMode ? '编辑波段做空策略' : '创建波段做空策略'}</span>
          <Tag color="purple">永续合约</Tag>
        </div>
      }
      open={open}
      onCancel={onCancel}
      ...
      footer={[
        ...
        <Button key="submit" type="primary" loading={loading} onClick={handleSubmit} danger>
          {editMode ? '更新策略' : '创建做空策略'}
        </Button>
      ]}
    >
      <Form
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <Row gutter={16} style={{ marginBottom: 16 }}>
            ...
            <Col span={12}>
              <Form.Item
                ...
                rules={[{ type: 'number', min: 0.01, max: 3, message: '偏移范围0.01%-3%' }]}
                tooltip="做空时挂高价，平仓时挂低价的百分比偏移量，模拟盘建议0.5%以上"
                style={{ marginBottom: 0 }}
              >
                <InputNumber
                  min={0.01}
          ...
          <Form.Item
            label={
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <LineChartDown size={14} style={{ marginRight: 4, color: '#1890ff' }} />
                <span>最低波动率要求 (%)</span>
              </div>
            }
            name="min_volatility"
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <Form.Item
            label={
              <div>
                <TrendingDown size={14} style={{ color: '#52c41a', marginRight: 4 }} />
                币价止盈比例 (%)
                {profitAmount && (
            ...
            tooltip="币价下跌达到此比例时止盈。例如，输入0.5，表示币价下跌0.5%时触发止盈。"
            style={{ marginBottom: 16 }}
          >
            <InputNumber
              min={0.1}
          ...
          <Form.Item
            label={
              <div>
                <TrendingUp size={14} style={{ color: '#ff4d4f', marginRight: 4 }} />
                币价止损比例 (%)
                {lossAmount && (
            ...
            tooltip="币价上涨达到此比例时止损。例如，输入0.2，表示币价上涨0.2%时触发止损。"
            style={{ marginBottom: 16 }}
          >
            <InputNumber
              min={0.1}
          ...
          <Form.Item
            label={
              <div>
                <RotateCw size={14} style={{ color: '#1890ff', marginRight: 4 }} />
                价格反弹比例 (%)
              </div>
            }
            name="reentry_pct"
            rules={[
              { required: true, message: '请输入反弹比例' },
              { type: 'number', min: 0.1, message: '反弹比例不能小于0.1%' }
            ]}
            tooltip="止盈后，价格从最低点反弹此比例时重新开空。例如，输入0.3，表示价格从低点上涨0.3%时卖出。"
            style={{ marginBottom: 0 }}
          >
            <InputNumber
              min={0.1}
